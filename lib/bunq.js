...
class Bunq {
  ...
  device({ ipAddresses, description }) {
    this.performRequest({
      method: 'POST',
  ...
  async session() {
    const data = await this.performRequest('POST', 'session-server', { secret: this.apiKey }, { 'X-Bunq-Client-Authentication': this.installationToken }, true);
